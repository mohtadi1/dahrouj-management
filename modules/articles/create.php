...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($code)) {
        $lastId = $db->query("SELECT MAX(id) FROM articles")->fetchColumn() + 1;
        $code = generateCode('ART', $lastId);
    }
    ...
    if (empty($name)) {
        ...
        $stmt = $db->prepare("INSERT INTO articles (code, name, description, category_id, purchase_price, sale_price, wholesale_price, quantity, min_stock, unit, image, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt->execute([$code, $name, $description, $category_id, $purchase_price, $sale_price, $wholesale_price, $quantity, $min_stock, $unit, $image, $status])) {
            $newId = $db->lastInsertId();
            logActivity('Création article', 'article', $newId);
...
<div class="page-header">
    <div>
        <h1 class="page-title"><i class="fas fa-boxes"></i> Nouvel Article</h1>
